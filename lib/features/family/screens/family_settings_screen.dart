...
class _FamilySettingsScreenState extends ConsumerState<FamilySettingsScreen> {
  ...
  void dispose() {
    ...
    _sloganCtrl.dispose(); _motdCtrl.dispose();
    super.dispose();
  }
    ...
    return Scaffold(body: Container(
      ...
      child: SafeArea(child: Column(children: [
        Padding(padding: const EdgeInsets.all(16), child: Row(children: [
          ...
          Text('Family Settings', style: AppTextStyles.headlineMedium),
        ])),
        Expanded(child: SingleChildScrollView(padding: const EdgeInsets.all(16), child: Column(
          crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            _field(_motdCtrl, 'MOTD (pinned announcement)', maxLines: 2),
            const SizedBox(height: 16),
            ...
            ...FamilyPrivacy.values.map((p) => GestureDetector(
              ...
              child: Container(margin: const EdgeInsets.only(bottom: 6), padding: const EdgeInsets.all(12),
                decoration: BoxDecoration(borderRadius: BorderRadius.circular(12),
                  ...
                  border: Border.all(color: _privacy == p ? AppColors.purpleNeon.withValues(alpha: 0.4) : AppColors.glassBorder)),
                child: Row(children: [
                  Icon(p.icon, color: _privacy == p ? AppColors.purpleGlow : AppColors.white30, size: 18),
                  const SizedBox(width: 10),
                  Text(p.displayName, style: TextStyle(color: _privacy == p ? AppColors.purpleGlow : AppColors.white70,
                    fontWeight: FontWeight.w600, fontSize: 13)),
                  const Spacer(),
                  if (_privacy == p) const Icon(Icons.check_circle, color: AppColors.purpleGlow, size: 18),
                ])),
            )),
            const SizedBox(height: 24),
            // Save
            GestureDetector(onTap: _save, child: Container(
              width: double.infinity, height: 48, decoration: BoxDecoration(
                borderRadius: BorderRadius.circular(14), gradient: AppGradients.purpleNeonGradient),
            ...
            const SizedBox(height: 16),
            // Admin links
            _adminLink(Icons.history, 'Audit Log', () => context.push('/family/audit')),
            _adminLink(Icons.swap_horiz, 'Transfer Ownership', () {}),
            _adminLink(Icons.delete_forever, 'Delete Family', () {}, color: AppColors.crimsonRed),
  ...
  Widget _sectionLabel(String text) => Padding(padding: const EdgeInsets.only(bottom: 6),
    child: Text(text, style: TextStyle(color: AppColors.white30, fontSize: 10,
      fontWeight: FontWeight.w700, letterSpacing: 1.5)));

  Widget _field(TextEditingController ctrl, String hint, {int maxLines = 1}) => TextField(
    controller: ctrl, maxLines: maxLines,
    style: const TextStyle(color: Colors.white, fontSize: 14),
    decoration: InputDecoration(hintText: hint, hintStyle: const TextStyle(color: AppColors.white30),
  ...
  Widget _adminLink(IconData icon, String label, VoidCallback onTap, {Color? color}) {
    return GestureDetector(onTap: onTap, child: Container(
      ...
      child: Row(children: [
        ...
        Expanded(child: Text(label, style: TextStyle(
          color: color ?? AppColors.white70, fontSize: 14, fontWeight: FontWeight.w500))),
  ...
  Future<void> _save() async {
    await ref.read(familyProvider.notifier).updateSettings(
      ...
      slogan: _sloganCtrl.text.trim(), motd: _motdCtrl.text.trim(),
      privacy: _privacy,
    );
    if (mounted) {
